...
?>
            <li<?php echo empty($class) ? '' : ' class="' . implode(' ', $class) . '"' ?>>
                <a href="?view&amp;s=<?php echo $server['id'] ?>&amp;key=<?php echo urlencode($fullkey) ?>"><?php echo format_html($name) ?><?php if ($len !== false) { ?>
        ...
        if (count($item) > 0) {
            ...
            <li class="folder<?php echo ($fullkey === '') ? '' : ' collapsed' ?><?php echo $islast ? ' last' : '' ?>">
                <div class="icon"><?php echo format_html($name) ?>&nbsp;<span class="info">(<?php echo count($item) ?>
                        )</span>
                    <?php if ($fullkey !== '' && $server['mode'] !== 'read-only'): ?>
                        <a href="<?php echo sprintf('delete.php?s=%s&amp;tree=%s:', $server['id'], urlencode($fullkey)); ?>" class="deltree">
