...
if($redis) {
    ...
    function print_namespace($item, $name, $fullkey, $islast) {
      ...
      if (count($item) > 0) {
        ...
        <?php if ($fullkey !== '' && !$server['is_read_only']) { ?><a href="delete.php?s=<?php echo $server['id']?>&amp;d=<?php echo $server['db']?>&amp;tree=<?php echo urlencode($fullkey)?>:" class="deltree"><img src="images/delete.png" width="10" height="10" title="Delete tree" alt="[X]"></a><?php } ?>
        </div><ul>
        <?php
